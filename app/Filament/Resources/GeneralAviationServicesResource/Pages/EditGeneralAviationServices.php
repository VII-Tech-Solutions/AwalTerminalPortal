...
class EditGeneralAviationServices extends EditRecord
{
    ...
    protected function callBeforeAndAfterSyncHooks($name, $value, $callback): void
    {
        ...
        if (Str::contains($name, [Attributes::SUBMISSION_STATUS_ID])) {
            $id = $this->data['id'];
            $general_aviation = GeneralAviationServices::query()->where(Attributes::ID, $id)->with('services')->first();
            ...
            $services = $general_aviation->services()->get();
...
//                        Attributes::GENERAL_AVIATION_ID => $service->id,
